"""
Trend Insights API Tests

Verifies the insights endpoint independently from MongoDB by
patching the business ownership guard and the underlying services.
"""
...
@pytest.mark.asyncio
async def test_insights_endpoint_returns_expected_insights(
    monkeypatch,
):
    """
    The insights endpoint should return deterministic insights based
    on a synthetic BusinessTrendReport.
    """
    ...
    assert (
        "publishing_activity" in codes
    )

    assert (
        "review_volume" in codes
    )